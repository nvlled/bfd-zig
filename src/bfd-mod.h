// This is a modified bfd.h. 
// The definitions of functions that take a bitfield'ed struct argument
// are moved elsewhere (to bfd-mod.c) The changes are made so that it works
// works on zig's C translation, in particular to workaround the 
// unsupported bitfield translation.
...
const char * bfd_section_name (const asection *sec);

bfd_size_type bfd_section_size (const asection *sec);

bfd_vma bfd_section_vma (const asection *sec);

bfd_vma bfd_section_lma (const asection *sec);

unsigned int bfd_section_alignment (const asection *sec);

flagword bfd_section_flags (const asection *sec);

void * bfd_section_userdata (const asection *sec);

bool bfd_is_com_section (const asection *sec);

/* Note: the following are provided as inline functions rather than macros
   because not all callers use the return value.  A macro implementation
   would use a comma expression, eg: "((ptr)->foo = val, TRUE)" and some
   compilers will complain about comma expressions that have no effect.  */
bool bfd_set_section_userdata (asection *sec, void *val);

bool bfd_set_section_vma (asection *sec, bfd_vma val);

bool bfd_set_section_lma (asection *sec, bfd_vma val);

bool bfd_set_section_alignment (asection *sec, unsigned int val);
...
bool bfd_is_und_section (const asection *sec);

bool bfd_is_abs_section (const asection *sec);

bool bfd_is_ind_section (const asection *sec);

bool bfd_is_const_section (const asection *sec);

/* Return TRUE if input section SEC has been discarded.  */
bool discarded_section (const asection *sec);
...
const char * bfd_get_filename (const bfd *abfd);

bool bfd_get_cacheable (const bfd *abfd);

enum bfd_format bfd_get_format (const bfd *abfd);

flagword bfd_get_file_flags (const bfd *abfd);

bfd_vma bfd_get_start_address (const bfd *abfd);

unsigned int bfd_get_symcount (const bfd *abfd);

unsigned int bfd_get_dynamic_symcount (const bfd *abfd);

struct bfd_symbol ** bfd_get_outsymbols (const bfd *abfd);

unsigned int bfd_count_sections (const bfd *abfd);

bool bfd_has_map (const bfd *abfd);

bool bfd_is_thin_archive (const bfd *abfd);

void * bfd_usrdata (const bfd *abfd);

/* See note beside bfd_set_section_userdata.  */
bool bfd_set_cacheable (bfd * abfd, bool val);

void bfd_set_thin_archive (bfd *abfd, bool val);

void bfd_set_usrdata (bfd *abfd, void *val);

asection * bfd_asymbol_section (const asymbol *sy);

bfd_vma bfd_asymbol_value (const asymbol *sy);

const char * bfd_asymbol_name (const asymbol *sy);

struct bfd * bfd_asymbol_bfd (const asymbol *sy);

void bfd_set_asymbol_name (asymbol *sy, const char *name);

/* For input sections return the original size on disk of the
   section.  For output sections return the current size.  */
bfd_size_type bfd_get_section_limit_octets (const bfd *abfd, const asection *sec);

/* Find the address one past the end of SEC.  */
bfd_size_type bfd_get_section_limit (const bfd *abfd, const asection *sec);
...
/* For input sections return the larger of the current size and the
   ...
   the current size.  */
bfd_size_type bfd_get_section_alloc_size (const bfd *abfd, const asection *sec);

/* Functions to handle insertion and deletion of a bfd's sections.  These
   only handle the list pointers, ie. do not adjust section_count,
   target_index etc.  */
void bfd_section_list_remove (bfd *abfd, asection *s);

void bfd_section_list_append (bfd *abfd, asection *s);

void bfd_section_list_prepend (bfd *abfd, asection *s);

void bfd_section_list_insert_after (bfd *abfd, asection *a, asection *s);

void bfd_section_list_insert_before (bfd *abfd, asection *b, asection *s);

bool bfd_section_removed_from_list (const bfd *abfd, const asection *s);
...
char * bfd_debug_name_to_zdebug (bfd *abfd, const char *name);

char * bfd_zdebug_name_to_debug (bfd *abfd, const char *name);
...
const char * bfd_get_target (const bfd *abfd);

enum bfd_flavour bfd_get_flavour (const bfd *abfd);

flagword bfd_applicable_file_flags (const bfd *abfd);

bool bfd_family_coff (const bfd *abfd);

bool bfd_big_endian (const bfd *abfd);
bool bfd_little_endian (const bfd *abfd);

bool bfd_header_big_endian (const bfd *abfd);

bool bfd_header_little_endian (const bfd *abfd);

flagword bfd_applicable_section_flags (const bfd *abfd);

char bfd_get_symbol_leading_char (const bfd *abfd);

enum bfd_flavour bfd_asymbol_flavour (const asymbol *sy);

bool bfd_keep_unused_section_symbols (const bfd *abfd);
